	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','tree','tree2','gridview'),
				'users'=>array('*'),
			),
	...
	public function actionTree2()
{
	if ( !$_GET['rooter'] && $_GET['root']=='source' ) {
			...
			elseif ($_GET['rooter'] && $_GET['root']=='source') {
				...
				$id_square = $_GET['rooter'];
				$id_pore = $_GET['root'];
				//$id_pore = '30BAC6A2-C955-564B-0407F96F6E0B     ';
					$archs=Yii::app()->db->createCommand()
					->select(' * ')
					->from('Fluid f')
				...
				elseif ($_GET['rooter'] && $_GET['root']=='source') {
					...
					foreach ($archs as $arch) {
						...
						$my_data[$arch['id']] = array(
							//'text'     => '<a href="#" onclick="tl_refresh_pores(\''.$arch['id'].'\')" title="">Скважина :'.$arch['no'].'</a>' ,
