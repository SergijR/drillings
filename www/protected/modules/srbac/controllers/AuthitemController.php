...
class AuthitemController extends SBaseController {
   ...
   */
	public $layout='//layouts/page/sidebar'; 
...
//  public $layout='//layouts/page/sidebar';
    ...
    if (isset($_POST['AuthItem'])) {
      ...
      try {
        ...
        Yii::app()->user->setFlash('updateError',
          ...
          . ' ' . $model->name . "<br />" .
          t( 'Possible there\'s already an item with the same name'));
        $this->renderPartial('manage/create', array('model' => $model));
      }
    } else {
      $this->renderPartial('manage/create', array('model' => $model));
  ...
  public function actionAutoDeleteItems() {
    ...
    $message = "<div style='font-weight:bold'>" . t( 'Delete operations') . "</div>";
    foreach ($actions as $key => $action) {
      ...
      if ($auth !== null) {
        $auth->delete();
        $message .= "<div>" . $action . " " . t( 'deleted') . "</div>";
      } else {
        $message .= "<div style='color:red;font-weight:bold'>" . t(
    ...
    if ($deleteTasks) {
      $message .= "<div style='font-weight:bold'>" . t( 'Delete tasks') . "</div>";
      foreach ($tasks as $key => $taskname) {
        $auth = AuthItem::model()->findByPk($taskname);
        if ($auth !== null) {
          $auth->delete();
          $message .= "<div>" . $taskname . " " . t( 'deleted') . "</div>";
        } else {
          $message .= "<div style='color:red;font-weight:bold'>" . t(
              'Error while deleting')
            . ' ' . $taskname . "</div>";
  ...
  public function actionAutoCreateItems() {
    ...
    if ($createTasks == "1") {
      $message = "<div style='font-weight:bold'>" . t( 'Creating tasks') . "</div>";
      foreach ($tasks as $key => $taskname) {
        $auth = new AuthItem();
        $auth->name = $taskname;
        $auth->type = 1;
        try {
          if ($auth->save()) {
            $message .= "'" . $auth->name . "' " .
              t( 'created successfully') . "<br />";
          } else {
            $message .= "<div style='color:red;font-weight:bold'>" . t(
                'Error while creating')
              . ' ' . $auth->name . '<br />' .
              t( 'Possible there\'s already an item with the same name') . "</div><br />";
          }
        } catch (Exception $e) {
          $message .= "<div style='color:red;font-weight:bold'>" . t(
              'Error while creating')
    ...
    $message .= "<div style='font-weight:bold'>" . t( 'Creating operations') . "</div>";
